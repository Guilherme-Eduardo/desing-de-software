    ...
    async function handleSubmit(e) {
        ...
        async function removerReserva(id) {
            try {
                ...
                if (!res.ok) {
                    throw new Error("Falha ao deletar reserva");
                }
        ...
        async function atualizarReserva(item) {
            try {
                await fetch(`http://localhost:3000/reserva/${item.id}`, {
                    method: "PATCH",
                });

                // Atualiza a lista no front removendo a reserva deletada
                setReserva((prev) => prev.filter((r) => r.id !== item.id));
        ...
        async function pagar(item) {
            try {
                await fetch(`http://localhost:3000/pagamento/${item.pagamento.id}`, {
                    method: "PATCH",
                });

            } catch (err) {
                console.error("Erro ao pagar reserva");
            }
        }
